...
COLS_DEFENSOR = ["Jugador", "Defensa", "Media Min", "Media PTS", "Media REB", "Media AST", "%FG Media"]
...
class NBADataLoader:
    ...
    def cargar_estadisticas_defensor(self, archivo_csv="resumen_cruce_jugador_defensor.csv"):
        """ Carga las estadísticas de los jugadores contra los defensores """
        try:
            self.df_defensor = pd.read_csv(archivo_csv)
            # Renombrar columnas para consistencia (cambio importante)
            self.df_defensor = self.df_defensor.rename(columns={
                "Jugador": "jugador",
                "Defensa": "defensor",
            ...
            # Limpiar nombres
            self.df_defensor = self.clean_string_columns(self.df_defensor, ["jugador", "defensor"])
            return self.df_defensor
        except Exception as e:
            logging.warning(f"No se pudo cargar estadísticas de defensores: {str(e)}")
            self.df_defensor = pd.DataFrame(columns=["jugador", "defensor", "media_min_def", "media_pts_def", "media_reb_def", "media_ast_def", "fg_perc_media_def"])
            return self.df_defensor
    ...
    def load_all_data(self):
        ...
        try:
            ...
            self.cargar_estadisticas_defensor()  # Carga las estadísticas del defensor
...
class FeatureEngineer:
    def __init__(self, df, df_j, df_e, df_p, df_defensor=None):
        self.df = df
        self.df_j = df_j
        self.df_e = df_e
        self.df_p = df_p
        self.df_defensor = df_defensor  # DataFrame de medias por defensor
    ...
    def add_rolling_features(self):
        ...
        # === NUEVO FEATURE: Media últimos 3 partidos ===
        for stat in ["pts", "reb", "ast"]:
            self.df[f"{stat}_last3"] = self.df.groupby('jugador')[stat].transform(lambda x: x.shift(1).rolling(3, min_periods=1).mean())

        # === NUEVO FEATURE: Media temporada hasta ese partido ===
        for stat in ["pts", "reb", "ast"]:
            self.df[f"{stat}_season_mean"] = self.df.groupby('jugador')[stat].transform(lambda x: x.expanding().mean().shift(1))
        
        # === NUEVO FEATURE: Racha respecto a la media de temporada ===
        for stat in ["pts", "reb", "ast"]:
            self.df[f"{stat}_streak_vs_season"] = self.df[f"{stat}_last3"] - self.df[f"{stat}_season_mean"]

        # === NUEVO FEATURE: Consistencia minutos últimos 5 partidos (std) ===
        self.df['min_std_last5'] = self.df.groupby('jugador')['min'].transform(lambda x: x.shift(1).rolling(5, min_periods=1).std())

        # === NUEVO FEATURE: Rolling mean minutos últimos 3 partidos ===
        self.df['min_last3'] = self.df.groupby('jugador')['min'].transform(lambda x: x.shift(1).rolling(3, min_periods=1).mean())

        # === NUEVO FEATURE: Split Home/Away para puntos ===
        self.df['pts_local_mean'] = self.df.groupby(['jugador', 'local'])['pts'].transform(lambda x: x.shift(1).expanding().mean())
    ...
    def _merge_all_data(self):
        ...
        # --------- MANTENEMOS EL BLOQUE DE DEFENSOR ---------
        if self.df_defensor is not None and not self.df_defensor.empty:
            defensor_means = self.df_defensor.groupby("jugador")[[
                "media_min_def", "media_pts_def", "media_reb_def", "media_ast_def", "fg_perc_media_def"
            ]].mean().reset_index()
            merged = merged.merge(defensor_means, on="jugador", how="left")
    ...
    def _select_features(self, merged_df):
        ...
        features = [
            ...
            'pts_last5_vs_opp', 'reb_last5_vs_opp', 'ast_last5_vs_opp', 'FG_perc_last5_vs_opp',
            'pts_last10', 'reb_last10', 'ast_last10', 'FG_perc_last10',
            'pts_last_season', 'reb_last_season', 'ast_last_season', 'FG_perc_last_season',
            'ptspp', 'rebpp', 'astpp',
            'local', 'dias_descanso_jugador', 'dias_descanso_rival', 'ultimo_resultado_equipo',
            'partidos_temporada', 'partidos_totales',
            # Features de defensor:
            'media_min_def', 'media_pts_def', 'media_reb_def', 'media_ast_def', 'fg_perc_media_def',
            # === NUEVOS FEATURES ===
            'pts_last3', 'reb_last3', 'ast_last3',
            'pts_season_mean', 'reb_season_mean', 'ast_season_mean',
            'pts_streak_vs_season', 'reb_streak_vs_season', 'ast_streak_vs_season',
            'min_std_last5', 'min_last3', 'pts_local_mean'
        ]
        for col in features:
            if col not in merged_df.columns:
                merged_df[col] = 0.0
        features_df = merged_df[features].copy()
        for col in ['media_min_def', 'media_pts_def', 'media_reb_def', 'media_ast_def', 'fg_perc_media_def']:
...
class NBAPredictor:
    ...
    def _prepare_features(self, input_data):
        ...
        defensor = input_data.get('defensor', "").upper().strip()
        try:
        ...
        features = {
            ...
            'ast_last5_vs_opp': last_data.get('ast_last5_vs_opp', DEFAULT_VALUES['ast']),
            ...
            'ast_last10': last_data.get('ast_last10', DEFAULT_VALUES['ast']),
            ...
            'ast_last_season': last_data.get('ast_last_season', DEFAULT_VALUES['ast']),
            ...
            'astpp': stats_pos.get('astpp', DEFAULT_VALUES['ast']),
        ...
        # Añadir features promedio vs defensor específico
        if self.feature_engineer.df_defensor is not None and defensor:
            mask = (
                (self.feature_engineer.df_defensor['jugador'] == jugador) &
                (self.feature_engineer.df_defensor['defensor'] == defensor)
            )
            if not self.feature_engineer.df_defensor[mask].empty:
                def_row = self.feature_engineer.df_defensor[mask].iloc[0]
                features.update({
                    'media_min_def': float(def_row['media_min_def']),
                    'media_pts_def': float(def_row['media_pts_def']),
                    'media_reb_def': float(def_row['media_reb_def']),
                    'media_ast_def': float(def_row['media_ast_def']),
                    'fg_perc_media_def': float(def_row['fg_perc_media_def'])
                })
            else:
                features.update({
                    'media_min_def': features.get('min', 0.0),
                    'media_pts_def': DEFAULT_VALUES.get('pts', 0.0),
                    'media_reb_def': DEFAULT_VALUES.get('reb', 0.0),
                    'media_ast_def': DEFAULT_VALUES.get('ast', 0.0),
                    'fg_perc_media_def': DEFAULT_VALUES.get('FG_perc', 0.0) if 'FG_perc' in DEFAULT_VALUES else 0.0
                })
        else:
            features.update({
                'media_min_def': features.get('min', 0.0),
        ...
        # ====================
        # NUEVO: FILLER DE FEATURES FALTANTES
        feature_order = list(self.feature_engineer.features.columns)
    ...
    def _get_last_game_data(self, jugador, rival):
        ...
        result = {
            ...
            'ast_last5_vs_opp': last5_vs_rival['ast'].mean() if not last5_vs_rival.empty else None,
            ...
            'ast_last_season': last_season['ast'].mean() if not last_season.empty else None,
...
def get_user_input():
    ...
    defensor = input("Defensor principal (ej. Anthony Davis, dejar vacío si no se conoce): ").strip()
...
def main():
    ...
    if opcion == '3':
        ...
        df, df_j, df_e, df_p, df_defensor = data_loader.load_all_data()
        feature_engineer = FeatureEngineer(df, df_j, df_e, df_p, df_defensor)
